...
export function AboutUs() {
  return (
    <div className="min-h-screen bg-[#EEF4FC] text-[#0b3566] font-sans antialiased selection:bg-[#0c6fc2] selection:text-white">
      {/* Top bar */}
      <header className="sticky top-0 z-40 bg-[#EEF4FC]/90 backdrop-blur-md border-b border-slate-200/60">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-display font-medium text-sm tracking-tight text-[#0b3566]">About Us</span>
          </div>
          <a href="/" className="flex items-center gap-1.5 font-sans text-xs uppercase tracking-[0.08em] text-slate-500 hover:text-[#0c6fc2] transition-colors">
      ...
      <section className="max-w-7xl mx-auto px-6 pt-16 md:pt-20 pb-12">
        ...
        >
          <span className="font-sans text-xs text-slate-400 uppercase tracking-[0.08em] block mb-3">Who we are</span>
          <h1 className="font-display font-light text-4xl md:text-6xl tracking-tight text-[#072750] leading-[1.08]">
      ...
      <section className="max-w-7xl mx-auto px-6 pb-24">
        <div className="border-t border-[#cbdff5] pt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {VALUES.map((v, i) => (
            <motion.div
              key={v.title}
            ...
            >
              <div className="w-9 h-9 rounded-full border border-[#cbdff5] bg-white flex items-center justify-center text-slate-700">
                <v.icon className="w-4 h-4" />
              </div>
              <h3 className="font-sans text-xs uppercase tracking-[0.08em] text-[#0b3566] font-semibold">{v.title}</h3>
