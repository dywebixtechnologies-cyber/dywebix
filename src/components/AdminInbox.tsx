...
export function AdminInbox({ onInquiryCountChange }: AdminInboxProps) {
  ...
  return (
    <section id="admin-section" className="py-24 px-6 bg-[#061c3a] border-t border-[#173a66]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-12">
          ...
          <div className="bg-[#0b2748] border border-[#173a66] p-6 rounded-md shadow-xs flex items-center justify-between">
            <div className="space-y-1">
              <span className="font-sans text-[11px] uppercase tracking-[0.08em] text-[#7f9cc4] block">REGISTERED USERS</span>
              <span className="font-sans font-bold text-3xl text-white leading-none block">{userCount}</span>
              <span className="text-[#8fabcf] text-xs block font-light">Client accounts created</span>
            </div>
            <div className="w-10 h-10 rounded bg-[#061c3a] flex items-center justify-center border border-[#173a66]/80">
              <Users className="w-4 h-4 text-[#a8c1e0]" />
            </div>
          </div>

          <div className="bg-[#0b2748] border border-[#173a66] p-6 rounded-md shadow-xs flex items-center justify-between">
            <div className="space-y-1">
              <span className="font-sans text-[11px] uppercase tracking-[0.08em] text-[#7f9cc4] block">TOTAL SUBMISSIONS</span>
              <span className="font-sans font-bold text-3xl text-white leading-none block">{totalVolume}</span>
              <span className="text-[#8fabcf] text-xs block font-light">Project ideas received</span>
            </div>
            <div className="w-10 h-10 rounded bg-[#061c3a] flex items-center justify-center border border-[#173a66]/80">
              <MessageSquare className="w-4 h-4 text-[#a8c1e0]" />
            </div>
          </div>

          <div className="bg-[#0b2748] border border-[#173a66] p-6 rounded-md shadow-xs flex items-center justify-between">
            <div className="space-y-1">
              <span className="font-sans text-[11px] uppercase tracking-[0.08em] text-[#7f9cc4] block">UNREAD MESSAGES</span>
              ...
              <span className="text-[#8fabcf] text-xs block font-light">Requires immediate follow-up</span>
            </div>
          ...
          <div className="bg-[#0b2748] border border-[#173a66] p-6 rounded-md shadow-xs flex items-center justify-between">
            <div className="space-y-1">
              <span className="font-sans text-[11px] uppercase tracking-[0.08em] text-[#7f9cc4] block">EARNED REVENUE</span>
              <span className="font-sans font-bold text-3xl text-white leading-none block">
                ₹{pipelineValue.toLocaleString('en-IN')}
              </span>
              <span className="text-[#8fabcf] text-xs block font-light">From finished projects only</span>
            </div>
            <div className="w-10 h-10 rounded bg-[#061c3a] flex items-center justify-center border border-[#173a66]/80">
              <IndianRupee className="w-4 h-4 text-[#a8c1e0]" />
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start" id="admin-workspace-split">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4">
            ...
            <div className="relative">
              <Search className="w-4 h-4 text-[#7f9cc4] absolute left-4 top-1/2 -translate-y-1/2" />
              <input
                type="text"
                placeholder="Search name, brief, budget..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full h-11 pl-11 pr-4 border border-[#173a66] text-xs rounded-md bg-[#0b2748] focus:border-[#0c6fc2] focus:outline-none"
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
              {searchedInquiries.length > 0 ? (
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 border rounded-md transition-all cursor-pointer flex flex-col justify-between group relative ${
                        isSelected 
                          ? 'border-[#0c6fc2] bg-[#061c3a] shadow-xs' 
                          : 'border-[#173a66] bg-[#0b2748] hover:border-[#2d5c94]'
                      }`}
                      id={`inq-item-${inq.id}`}
                    >
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        <span className="font-sans text-[11px] bg-[#061c3a] border border-[#173a66]/80 text-[#a8c1e0] px-1.5 py-0.5 rounded">
                          {inq.id}
                        </span>
                        <span className="font-sans text-[11px] text-[#7f9cc4]">
                          {new Date(inq.timestamp).toLocaleDateString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                        </span>
                      </div>

                      <div className="text-sm font-semibold text-white group-hover:text-[#0c6fc2] truncate mb-1">
                        {inq.name}
                        {inq.company && <span className="text-xs text-[#8fabcf] font-light"> at {inq.company}</span>}
                      </div>

                      <div className="text-xs text-[#8fabcf] font-light line-clamp-1 mb-2">
                        {inq.details}
                      </div>

                      <div className="flex items-center justify-between border-t border-[#173a66] pt-2 mt-2">
                        <span className="text-xs font-sans font-medium text-[#8fabcf]">
                          {inq.projectType}
                        </span>
                        ...
                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            className="p-1 text-[#7f9cc4] hover:text-[#0c6fc2] font-sans text-[11px] uppercase tracking-wide cursor-pointer focus:outline-none"
                          ...
                          <button
                            ...
                            className="p-1 text-[#7f9cc4] hover:text-red-500 cursor-pointer focus:outline-none"
                ...
                <div className="text-center py-12 border border-dashed border-[#173a66] rounded text-[#7f9cc4] font-sans text-xs">
          ...
          <div className="lg:col-span-7 bg-[#0b2748] border border-[#173a66] p-8 rounded-md shadow-xs min-h-[460px]">
            <AnimatePresence mode="wait">
              {selectedInquiry ? (
                ...
                >
                  ...
                  {/* Header info */}
                  <div className="space-y-1.5 border-b border-[#173a66] pb-4">
                    <div className="flex items-center justify-between">
                      <span className="font-sans text-[11px] text-[#7f9cc4]">INSPECTING BLUEPRINT</span>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-sans text-[11px] bg-[#0c6fc2] text-white px-2 py-0.5 rounded font-bold uppercase tracking-wider">{selectedInquiry.id}</span>
                      </div>
                    </div>
                    <h3 className="font-sans font-semibold text-2xl text-white tracking-tight leading-none mt-2">
                       {selectedInquiry.name}
                    </h3>
                    <div className="text-[#8fabcf] font-sans text-xs flex flex-wrap items-center gap-3">
                      <span>{selectedInquiry.email}</span>
                      {selectedInquiry.company && (
                        <>
                          <span className="text-[#6e8cb4]">|</span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-[#061c3a] p-4 rounded border border-[#173a66] font-sans text-xs text-[#a8c1e0]">
                    <div className="space-y-1.5">
                      <span className="text-[11px] text-[#7f9cc4] block uppercase tracking-wider">Scope Architecture</span>
                      <span className="text-white font-medium block">{selectedInquiry.projectType}</span>
                    </div>
                    <div className="space-y-1.5">
                      <span className="text-[11px] text-[#7f9cc4] block uppercase tracking-wider">Logged On</span>
                  ...
                  <div className="bg-[#0b2748] border border-[#173a66] rounded p-4 space-y-2">
                    <span className="font-sans text-[11px] text-[#7f9cc4] uppercase tracking-wider block">Project Rate (visible to the client)</span>
                    <div className="flex items-center gap-2">
                      <div className="flex items-center gap-1.5 flex-1 border border-[#173a66] rounded px-3 h-10 bg-[#061c3a]/40 focus-within:border-[#0c6fc2] transition-colors">
                        <IndianRupee className="w-3.5 h-3.5 text-[#7f9cc4] shrink-0" />
                        <input
                          type="text"
                          inputMode="numeric"
                      ...
                      <button
                        ...
                        className="rounded bg-[#0c6fc2] text-white text-xs font-sans tracking-[0.08em] uppercase px-4 h-10 hover:bg-[#0b2748] transition-all cursor-pointer"
                        id="inq-set-rate-btn"
                      >
                        Save
                      </button>
                    ...
                    {selectedInquiry.accepted && selectedInquiry.acceptedAt && (
                      <span className="font-sans text-xs text-emerald-600 block">
                  ...
                  <div className="flex-1 space-y-2">
                    <span className="font-sans text-[11px] uppercase tracking-[0.08em] text-[#7f9cc4] block">PROJECT SCOPE SPECIFICATION SUMMARY:</span>
                    <div className="p-5 border border-[#173a66] rounded text-sm text-[#c3d6ee] leading-relaxed font-light whitespace-pre-wrap">
                      {selectedInquiry.details}
                    </div>
                  </div>

                  {/* Actions Drawer */}
                  <div className="border-t border-[#173a66] pt-6 flex flex-wrap items-center justify-between gap-4">
                    <div className="flex flex-wrap gap-3">
                      <button
                        onClick={(e) => handleToggleAccept(selectedInquiry.id, e)}
                        className={`flex items-center gap-1.5 rounded px-4 py-2 font-sans text-xs transition-all cursor-pointer ${
                          selectedInquiry.accepted
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 border border-[#173a66] rounded px-4 py-2 hover:bg-[#061c3a] font-sans text-xs text-[#a8c1e0] transition-all cursor-pointer"
                        id="inq-read-toggle-btn-pane"
                      >
                        <CheckSquare className="w-4 h-4 text-[#7f9cc4]" />
                        {selectedInquiry.read ? 'Mark as Unread' : 'Mark as Read'}
                      </button>
                      <button
                        onClick={(e) => handleDelete(selectedInquiry.id, e)}
                        className="flex items-center gap-1.5 border border-[#173a66] hover:border-red-400 hover:text-red-500 hover:bg-red-50/10 rounded px-4 py-2 font-sans text-xs text-[#a8c1e0] transition-all cursor-pointer"
                        id="inq-delete-btn-pane"
                      >
                        <Trash2 className="w-4 h-4 text-[#7f9cc4] group-hover:text-red-500" />
                        Delete Inquiry
                      </button>
                    </div>
                    ...
                    <a
                      ...
                      className="rounded bg-[#0c6fc2] hover:bg-[#0b2748] text-white text-xs font-sans tracking-[0.08em] uppercase py-3.5 px-6 flex items-center justify-center gap-2 cursor-pointer transition-all"
                ...
                <div className="py-24 text-center text-[#7f9cc4] font-sans text-xs flex flex-col items-center justify-center gap-4">
                  <BookOpen className="w-6 h-6 text-[#6e8cb4]" />
