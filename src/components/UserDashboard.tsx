    ...
    <div className="min-h-screen bg-[#F8F9FA] text-slate-900 font-sans antialiased">
      ...
      <header className="sticky top-0 z-40 bg-[#F8F9FA]/90 backdrop-blur-md border-b border-slate-200/60">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5 text-slate-900">
            ...
            <span className="font-display font-medium text-sm tracking-tight text-slate-900">My Dashboard</span>
          </div>
          <div className="flex items-center gap-4">
            <a href="/" className="hidden sm:flex items-center gap-1.5 font-sans text-xs uppercase tracking-[0.08em] text-slate-500 hover:text-black transition-colors">
              <ArrowLeft className="w-3 h-3" /> View site
            </a>
            <button
              onClick={handleLogout}
              className="flex items-center gap-1.5 border border-slate-200 rounded px-3 py-1.5 hover:bg-slate-100 font-sans text-xs uppercase tracking-[0.08em] text-slate-600 transition-all cursor-pointer"
              id="user-logout-btn"
            >
      ...
      <section className="py-16 px-6">
        <div className="max-w-7xl mx-auto">
          {/* Greeting */}
          <div className="mb-12">
            <span className="font-sans text-xs text-slate-400 uppercase tracking-[0.08em] block mb-3">Your projects</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <div className="bg-white border border-slate-200 p-6 rounded-md shadow-xs flex items-center justify-between">
              <div className="space-y-1">
                <span className="font-sans text-[11px] uppercase tracking-[0.08em] text-slate-400 block">PROJECTS REQUESTED</span>
                <span className="font-sans font-bold text-3xl text-slate-900 leading-none block">{total}</span>
                <span className="text-slate-500 text-xs block font-light">Ideas you've submitted</span>
              </div>
            ...
            <div className="bg-white border border-slate-200 p-6 rounded-md shadow-xs flex items-center justify-between">
              <div className="space-y-1">
                <span className="font-sans text-[11px] uppercase tracking-[0.08em] text-slate-400 block">ACCEPTED BY ADMIN</span>
                <span className="font-sans font-bold text-3xl text-[#10b981] leading-none block">{acceptedCount}</span>
                <span className="text-slate-500 text-xs block font-light">Approved & in the pipeline</span>
              </div>
            ...
            <div className="bg-white border border-slate-200 p-6 rounded-md shadow-xs flex items-center justify-between">
              <div className="space-y-1">
                <span className="font-sans text-[11px] uppercase tracking-[0.08em] text-slate-400 block">AWAITING REVIEW</span>
                <span className="font-sans font-bold text-3xl text-slate-900 leading-none block">{pendingCount}</span>
                <span className="text-slate-500 text-xs block font-light">Pending the studio's response</span>
              </div>
              <div className="w-10 h-10 rounded bg-slate-50 flex items-center justify-center border border-slate-200/80">
          ...
          <div className="flex items-center justify-between mb-6">
            <h2 className="font-sans text-xs uppercase tracking-[0.08em] text-slate-900 font-semibold">Your Project Ideas</h2>
            <button
              onClick={goToContact}
              className="flex items-center gap-2 rounded-full bg-black h-10 px-5 text-white hover:bg-slate-900 font-sans text-xs tracking-[0.08em] uppercase transition-all cursor-pointer"
            >
          ...
          {projects.length === 0 ? (
            <div className="bg-white border border-dashed border-slate-300 rounded-md p-12 text-center flex flex-col items-center gap-4">
              ...
              <button
                onClick={goToContact}
                className="flex items-center gap-2 font-sans text-xs uppercase tracking-[0.08em] text-black border-b border-slate-300 pb-0.5 hover:gap-3 transition-all cursor-pointer"
              >
                  ...
                  >
                    <div className="flex-1 min-w-0 space-y-1.5">
                      <div className="flex items-center gap-2">
                        <span className="font-sans text-[11px] bg-black text-white px-2 py-0.5 rounded font-bold uppercase tracking-wider">{p.id}</span>
                        {p.finished ? (
                    ...
                    <div className="flex items-center gap-6 md:gap-8 shrink-0 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-8">
                      <div className="space-y-0.5">
                        <span className="font-sans text-[11px] text-slate-400 uppercase tracking-wider block">Rate</span>
                      ...
                      <div className="space-y-0.5">
                        <span className="font-sans text-[11px] text-slate-400 uppercase tracking-wider block">Since Accepted</span>
                        <span className="text-slate-900 text-sm font-semibold flex items-center gap-1">
